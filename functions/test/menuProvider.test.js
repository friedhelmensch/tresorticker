...
it("Parsing the text representation of Wochenkarte should create menu", () => {
    ...
    const menuText = fs
        .readFileSync("./test/Wochenkarte.content.txt", "utf8")
        .replace(/(\r\n |\n |\r )/gm, "");
    ...
    const expectedMenu = [
        {
            day: "2018-02-26",
            meals: ["Schupfnudelpfanne mit Sauerkraut und Kasseler € 7,50"]
        },
        {
            day: "2018-02-27",
            meals: ["Gebratene Hähnchenbrust auf Gemüsereis und Magodip € 7,50"]
        },
        {
            day: "2018-02-28",
            meals: ["Gefüllter Schweinehals mit Spätzle € 7,50"]
        },
        {
            day: "2018-03-01",
            meals: [
        ...
        {
            day: "2018-03-02",
            meals: ["Tilapiafilet mit Rosmarinkartoffeln € 7,50"]
        },
        {
            day: "Täglich",
            meals: [
...
it("GetMenu should return daily special as well as all week daily dishes", () => {
    ...
    const menuText = fs
        .readFileSync("./test/Wochenkarte.content.txt", "utf8")
        .replace(/(\r\n |\n |\r )/gm, "");
    const todaysMenu = menuProvider.getMenuByDay(
...
it("When menus week range is not complete get menu should return the right menu based on the remaining dates", () => {
    ...
    const menuText = fs
        .readFileSync("./test/Wochenkarte.DateRange.content.txt", "utf8")
        .replace(/(\r\n |\n |\r )/gm, "");
    const todaysMenu = menuProvider.getMenuByDay(
    ...
    const expectedDailyMenu = [
        {
            day: "2018-04-05",
            meals: ["Putenrollbraten mit Spätzle € 7,50"]
        },
        {
            day: "Täglich",
            meals: [
...
it("GetMenu should in weeks with holidays", () => {
    ...
    const menuText = fs
        .readFileSync("./test/Wochenkarte.Feiertag.content.txt", "utf8")
        .replace(/(\r\n |\n |\r )/gm, "");
    const todaysMenu = menuProvider.getMenuByDay(
...
it("Extracting start date of weekly menu should return 02.07.2018 (NoSpaceInDate)", () => {
    ...
    const menuText = fs
        .readFileSync("./test/Wochenkarte.NoSpaceInDate.content.txt", "utf8")
        .replace(/(\r\n |\n |\r )/gm, "");
...
it("should return correct meals even start date is a sunday instead of monday", () => {
    const fs = require("fs");
    const LocalDate = require("js-joda").LocalDate;

    const menuText = fs
        .readFileSync("./test/Wochenkarte.WrongWeekRange.content.txt", "utf8")
        .replace(/(\r\n |\n |\r )/gm, "");
    ...
    const expectedDailyMenu = [
        ...
        },
        {
            day: "Täglich",
            meals: [
                "Salatteller mit gebratenem Pangasiusfilet € 7,60",
                "Champignonomelett mit Salatbukett € 6,80",
                "Kalbsbratwurst mit Kartoffelsalat € 7,20"
            ]
        }
    ];
    expect(todaysMenu).to.deep.equal(expectedDailyMenu);
